...
def  main():
  ...
  installer = "\installer\Haste Installer.exe"
   
  if demo:
    installer = "\installer\Haste Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Haste manual.pdf" 
  ]
  ...
  ZIPNAME = "Haste-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Haste-v" + FULLVERSIONSTR + "-win-demo.zip"
